...
def test_net(args):
    ...
    for epoch in range(76,77,5):
        
        num = str(epoch)
        model_path = args.save_model_dir + num + 'RGB_VST_rcmt_cross.pth'
        print(model_path)
        ...
        for i, data_batch in enumerate(test_loader):
            ...
            dataset = image_path[0].split('/')[-2].split('_')[-1]
            filename = image_path[0].split('/')[-1]
            # print(filename)
            # save saliency maps
            sdir = args.test_dir
            sset = args.test_set
            save_test_path = args.save_test_path_root + '/' + sdir + '/' + sset + '/'
            # save_test_path = args.save_test_path_root + dataset + '/RGB_VST3/'
            save_test_path = os.path.join(save_test_path, dataset)
            
            if not os.path.exists(save_test_path):
                os.makedirs(save_test_path)
            if '.jpg' in filename:
                output_s.save(os.path.join(save_test_path,filename.replace('.jpg','.png') ))
            else:
                output_s.save(os.path.join(save_test_path,filename ))
